...
def start_recording(
    camera_id, output_path, width, height, fps, duration_sec, shutter_us, gain
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Record 60s videos from Pi cameras 0 and 1 using rpicam-vid."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gain", type=float, default=2.0, help="Analog gain (default: 2.0)"
    )
    parser.add_argument(
        "--out-dir",
        default="recordings",
    ...
    if ok0 and ok1:
        ...
        if (frames0 is not None and frames0 <= 1) or (
            frames1 is not None and frames1 <= 1
        ):
            print(
                "[WARN] At least one output still has <= 1 frame. Try lower --w/--h or --fps."
            )
        return
