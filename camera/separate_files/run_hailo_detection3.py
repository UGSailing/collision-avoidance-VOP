...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.epilog = (
        "Example:\n"
        f"  {camera_dir / 'run_hailo_detection3.py'} --input rpi --object-height 0.23\n"
        f"  {camera_dir / 'run_hailo_detection3.py'} --input /dev/video0 --object-height 0.23"
    )
    return parser
...
def draw_distance_overlay(frame: np.ndarray, estimates: list[DetectionEstimate]) -> None:
    for estimate in estimates:
        ...
        text = f"{estimate.distance_m:.2f}m | {estimate.angle_deg:.2f}deg"
        cv2.putText(
            frame,
            text,
            (x1, max(y1 - 28, 45)),
