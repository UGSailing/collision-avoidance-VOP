...
### met PNG's:
### python3 calibrations/rectification.py --calib calib_cam0.yaml --input-dir calib_good_calibration_imgs --glob "*.png" --out captures --alpha 0
...
def process_existing_images(
    input_dir, pattern, out_dir, w, h, map1, map2, roi, alpha, show_undist_only=False
):
    import glob
...
def main():
    ...
    ap.add_argument(
        "--calib", default="calib_cam0.yaml", help="Path to calibration YAML"
    )
    ap.add_argument(
        "--camera", type=int, default=0, help="Camera index for rpicam-still"
    )
    ap.add_argument("--out", default="captures", help="Output folder")
    ap.add_argument(
        "--alpha", type=float, default=0.0, help="0=crop, 1=keep all (black borders)"
    )
    ap.add_argument(
        "--settle-ms", type=int, default=1500, help="AE/AWB settle time for each still"
    )
    ap.add_argument(
        "--w", type=int, default=0, help="Override width (0 = use calib width)"
    )
    ap.add_argument(
        "--h", type=int, default=0, help="Override height (0 = use calib height)"
    )
    ap.add_argument(
        "--show-undist-only", action="store_true", help="Show only undistorted view"
    )
    ap.add_argument(
        "--extra", default="", help="Extra rpicam-still args as one string (advanced)"
    )
    ap.add_argument(
        "--input-dir",
        default="",
        help="Process existing images from this folder instead of live capture",
    )
    ...
    print(
        f"[INFO] rpicam camera={args.camera}, size={w}x{h}, alpha={args.alpha}, out='{args.out}'"
    )
    ...
    cv2.putText(
        last_disp,
        "Press 'c' to capture (rpicam-still)",
        (30, 60),
    ...
    while True:
        ...
        if k == ord("c"):
            ...
            try:
                capture_one_rpicam(
                    args.camera, raw_path, w, h, args.settle_ms, extra_args
                )
