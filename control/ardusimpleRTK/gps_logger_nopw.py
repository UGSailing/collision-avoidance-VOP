#!/usr/bin/env python3
"""
UM982 (Unicore) — 20 Hz NMEA + (optional) NTRIP + Minimal CSV + Keyboard control (Raspberry Pi OS)

Outputs to STDOUT (CSV):
  utc_time_iso,fix_type,lat_deg,lon_deg,true_heading_deg

Keyboard (read from the terminal in raw mode):
  's' -> toggle file logging on/off (writes the same CSV rows to gnss_minimal_*.csv)
  'e' -> cleanly stop the program

All status/info messages (setup, errors, NTRIP, toggle notices) go to STDERR so STDOUT stays clean CSV.
...
# For keyboard control (Linux/Unix terminals)
import termios
import tty
import select

# =========================
# Utility: STDERR printing
# =========================

def eprint(*args, **kwargs):
    """Print to STDERR to avoid polluting CSV on STDOUT."""
...
# ---- Output cadence ----
PRINT_HZ = 20  # target CSV print rate (Hz)

# If antenna bar not perfectly aligned with vehicle axis, apply offset
...
# State for incoming parsed fields
...
# Logging control shared between keyboard and printer
log_ctrl_lock = threading.Lock()
log_request = False  # user toggles via 's'
# The printer will open/close the file to reflect log_request.
...
def setup_um982_to_20hz_com1_heading():
    eprint(f"[setup] Opening {PORT} @ {BAUD} ...")
    try:
        ser = open_port(PORT, BAUD, timeout=0.2)
    except Exception as e:
        eprint(f"[setup] Could not open {PORT}: {e}")
        eprint("Hint: On Raspberry Pi, ensure your user is in the 'dialout' group and the device path is correct.")
        sys.exit(1)
    ...
    if RESET_ON_START:
        ...
        drain_for(ser, 0.2)
        ser.close()
        eprint("[setup] Factory reset sent. Waiting for receiver to reboot ...")
        time.sleep(10.0)
        # Re-open after reboot
        for _ in range(12):
            try:
                ser = open_port(PORT, BAUD, timeout=0.2)
                eprint(f"[setup] Reconnected to {PORT}.")
                break
            except Exception:
                time.sleep(0.7)
        else:
            eprint("[setup] Failed to reconnect after reset.")
            sys.exit(1)
        drain_for(ser, 0.5)

    # Enable rover + heading mode, set COM1 baud, enable 20 Hz NMEA
    ...
    write_cmd(ser, f"CONFIG COM3 {COM1_BAUD}")     # Align COM3 for shield / side link

    # 20 Hz sentences (period = 0.05 s)
    period = 0.05  # 20 Hz
    ...
    if SETUP_ENABLE_VTG:
        write_cmd(ser, f"GNVTG COM1 {period}")  # 10 Hz example
        write_cmd(ser, f"GNVTG COM3 {period}")

    write_cmd(ser, f"SAVECONFIG")
    drain_for(ser, 0.5)
    ser.close()
    eprint("[setup] Configuration done and saved (on COM1 and COM3).")
    ...
    while not stop_evt.is_set():
        ...
        try:
            ...
            if "401" in first_line:
                eprint("[ntrip] 401 Unauthorized: check username/password.")
                sock.close(); sock = None
                time.sleep(3); continue
            if "404" in first_line:
                eprint("[ntrip] 404 Not Found: mountpoint incorrect or case mismatch.")
                sock.close(); sock = None
                time.sleep(5); continue
            if not ("200" in first_line or "ICY 200" in first_line):
                eprint("[ntrip] Bad response:\n" + head_text)
                sock.close(); sock = None
                time.sleep(3); continue

            eprint(f"[ntrip] Connected to mount '{NTRIP_MOUNT}'; streaming RTCM...")
            if rest:
                rtcm_q.put(rest)
            ...
            eprint(f"[ntrip] Reconnecting in 3 s: {e}")
            try:
                if sock: sock.close()
            except Exception:
                pass
...
def run_csv_printer(stop_evt):
    """
    Print ONLY:
      utc_time_iso,fix_type,lat_deg,lon_deg,true_heading_deg

    - CSV header printed once (to STDOUT)
    - Same rows optionally written to a file when log_request == True (toggled by 's')
    """
    FRAME_DT = 1.0 / max(1, PRINT_HZ)
    next_t = time.perf_counter() + FRAME_DT

    header_printed = False
    logging_enabled = False  # actual file-open state
    log_fh = None
    ...
    while not stop_evt.is_set():
        ...
        if now >= next_t:
            # reflect user logging request
            with log_ctrl_lock:
                req = log_request

            if req and not logging_enabled:
                # open new file and write header
                ts = time.strftime("%Y%m%d_%H%M%S", time.gmtime())
                fname = f"gnss_minimal_{ts}.csv"
                try:
                    log_fh = open(fname, "w", buffering=1)
                    print("utc_time_iso,fix_type,lat_deg,lon_deg,true_heading_deg", file=log_fh)
                    logging_enabled = True
                    eprint(f"[log] Started file logging -> {fname}  (press 's' to stop)")
                except Exception as e:
                    eprint(f"[log] Failed to open log file: {e}")
                    logging_enabled = False
                    log_fh = None

            if not req and logging_enabled:
                # close current file
                try:
                    log_fh.flush()
                    log_fh.close()
                ...
                logging_enabled = False
                log_fh = None
                eprint("[log] Stopped file logging (press 's' to start again)")

            # snapshot state
            with state_lock:
                ts_iso = format_time_iso(state["utc_date"], state["utc_time"])
                fix    = state["fix"] or "-"
                nsats  = state["nsats"]
                lat    = state["lat"]
                lon    = state["lon"]
            ...
            # print header once to STDOUT
            if not header_printed:
                print("utc_time_iso,fix_type,lat_deg,lon_deg,true_heading_deg")
                header_printed = True

            # CSV line (STDOUT)
            line = ",".join([
                ts_iso if ts_iso else "-",
                str(fix)+str(nsats),
                fmt_num(lat, 7),
                fmt_num(lon, 7),
                fmt_num(hdg, 1),
            ...
            if logging_enabled and log_fh:
                try:
                    print(line, file=log_fh)
                except Exception as e:
                    eprint(f"[log] Write error: {e}")

            next_t += FRAME_DT
            if now > next_t + FRAME_DT:
                # if we fell behind, re-align
                next_t = now + FRAME_DT
...
def keyboard_thread(stop_evt):
    """
    Raw, non-blocking keyboard:
      's' -> toggle log_request
      'e' -> set stop_evt
    """
    fd = sys.stdin.fileno()
    try:
        ...
        eprint("[key] Warning: not a TTY; keyboard controls disabled.")
        return

    tty.setcbreak(fd)
    eprint("[key] Keyboard: press 's' to start/stop file logging, 'e' to exit.")
        ...
        while not stop_evt.is_set():
            ...
            if r:
                ...
                if c == 's':
                    with log_ctrl_lock:
                        global log_request
                        log_request = not log_request
                    eprint(f"[key] Toggled logging -> {'ON' if log_request else 'OFF'}")
                elif c == 'e':
                    eprint("[key] Exit requested (e).")
                    stop_evt.set()
                    break
    finally:
...
def main():
    # 1) Configure UM982 (20 Hz GGA/RMC/HDT on COM1/COM3)
    setup_um982_to_20hz_com1_heading()
    ...
    # 3) CSV printer (main thread stays alive)
    try:
        run_csv_printer(stop_evt)
    except KeyboardInterrupt:
        pass
    finally:
        stop_evt.set()
